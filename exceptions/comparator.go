...
var (
	...
	setGlobalComparatorOnce sync.Once
)

// newComparator builds a comparator.
//
// Since the comparator holds a cache, this builds a single global instance only on the first call.
//
// TODO(fred): it would be better to avoid using a global cache and make this a part of some
// "ExceptionProcessor" struct. However, this implies quite a few changes in the interface, so
// I suggest to defer such breaking changes to a later PR.
func newComparator() *comparator {
	setGlobalComparatorOnce.Do(func() {
		globalComparator = &comparator{}
	})
